...
def flow():
    ...
    posts = db.paginate(query, page=page,
                        per_page=app.config['POSTS_PER_PAGE'], error_out=False)
    next_url = url_for('flow', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('flow', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', title='Поток', posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Инструкция отправлена. Проверьте свой почтовый ящик.')
        return redirect(url_for('login'))
